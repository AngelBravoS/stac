...
/*
  ...
  sin necesitar otro método recibe el exponente de cada variable así como
  su índice dentro de la matriz (0, 1, 2...)

  como las dos variables tienen la misma longitud
  (vector[0].length = vector[1].length) y solo hay que indicar
...
double sumatoria2Var(Matriz &m1, int exponenteX, int exponenteY) {
  ...
  for (unsigned int i = 0; i < n; i++) {
    sumatoria2Variables +=
        pow(m1[0][i], exponenteX) * pow(m1[1][i], exponenteY);
  }
