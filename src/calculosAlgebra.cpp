...
double determinante(Matriz &m1) {
	...
	if(m1.cuadrada == false) {
		...
		switch(size) {
			...
			case 3://por sarrus
				resultado = (m1[0][0] * m1[1][1] * m1[2][2] +
			...
			default :
				for(unsigned int j = 0; j < size; j++) {
					Matriz menoradj = menor(m1, 0, j);    // Eliminamos la fila 0 y la columna j
		...
		default:
			for(unsigned int i = 0; i < size; i++) {
				for(unsigned int j = 0; j < size; j++) {
					...
					// Calculamos el cofactor como el determinante del menor, ajustado por el signo
